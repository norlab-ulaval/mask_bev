...
class TestKittiDataModule(unittest.TestCase):
    def setUp(self):
        pl.seed_everything(42)
        self.batch_size = 2

    def test_tensor_batch(self):
        datamodule = KittiDataModule('~/Datasets/KITTI', self.batch_size, 1, 50, (0, 70.4), (-40, 40), (-20, 20),
        ...
        # (B, num_queries Nx, Ny)
        self.assertEqual((self.batch_size, datamodule.num_queries, 500, 440), masks.shape)
    ...
    def test_list_batch_num_queries_train(self):
        ...
        for pc, (labels, masks) in dataloader:
            self.assertEqual((self.batch_size, datamodule.num_queries), labels.shape)
            self.assertEqual((self.batch_size, datamodule.num_queries, 440, 500), masks.shape)
