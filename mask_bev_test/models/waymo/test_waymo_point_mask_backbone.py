...
class TestWaymoPointMaskBackbone(unittest.TestCase):
    def setUp(self):
        self.batch_size = 2
        self.channels = 64
        x_range, y_range, z_range, voxel_size = (-40, 40), (-40, 40), (-10, 10), 0.5
    ...
    def test_backbone(self):
        ...
        expected_sizes = [40, 20, 10, 5]
        expected_dim = [2 ** i * 96 for i in range(4)]
        for i, x in enumerate(out):
            self.assertEqual((self.batch_size, expected_dim[i], expected_sizes[i], expected_sizes[i]), x.shape)
