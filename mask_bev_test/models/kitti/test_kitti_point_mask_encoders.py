...
class TestPointMaskEncoders(unittest.TestCase):
    def setUp(self):
        self.batch_size = 1
        z_range = (-20, 20)
        x_range, y_range, z_range, voxel_size = (0, 70.4), (-40, 40), z_range, 0.16
        torch.random.manual_seed(4)
        random.seed(4)
        self.datamodule = KittiDataModule('data/KITTI', self.batch_size, 1, 45,
                                          x_range, y_range, z_range, voxel_size, True, 0, False,
                                          collate_fn=CollateType.ListCollate, shuffle_train=False)
    ...
    def test_voxelization(self):
        ...
        # voxels [V, max_point, 4] -> (x, y, z, r)
        # the voxel at index i contains num_points[i] points, the rest is 0
        num_voxels = voxels.shape[0]
        self.assertEqual((num_voxels, self.max_num_points, 4), voxels.shape)
    ...
    def test_forward(self):
        ...
        self.assertEqual((self.batch_size, self.channels, self.num_voxel_y, self.num_voxel_x), features.shape)
