# Inspired by https://github.com/valeoai/LightConvPoint/blob/0c52645be16859eb305bdec7514e546253f1ebc2/lightconvpoint/nn/conv_fkaconv.py
...
class FKAConv(nn.Module):
    ...
    def forward(self, input, points, support_points):
        ...
        num_pts = input.shape[2]
        # center the neighborhoods (local coordinates)
        pts = points - support_points
        ...
        if self.training:
            mean_radius = distances.max(1)[0].mean()
        ...
        distance_weight_s = distance_weight.sum(1, keepdim=True)
        distance_weight_s = distance_weight_s + (distance_weight_s == 0) + 1e-6
        distance_weight = (
                distance_weight / distance_weight_s * distances.shape[1]
        )
        distance_weight = einops.repeat(distance_weight, 'b p -> b k p', k=self.kernel_size)

        # feature weighting matrix estimation
        mat = F.relu(self.bn1(self.fc1(pts.unsqueeze(2)))).squeeze()
        mp1 = torch.max(mat * distance_weight, dim=2, keepdim=True)[0]
        mp1 = einops.repeat(mp1, 'b k 1 -> b k p', p=num_pts)
        mat = torch.cat([mat, mp1], dim=1)
        mat = F.relu(self.bn2(self.fc2(mat.unsqueeze(2))))
        mp2 = torch.max(mat.squeeze() * distance_weight, dim=2, keepdim=True)[0]
        mp2 = einops.repeat(mp2, 'b k 1 -> b k p', p=num_pts)
        mat = torch.cat([mat.squeeze(), mp2], dim=1)
        mat = F.relu(self.fc3(mat.unsqueeze(2))).squeeze() * distance_weight

        # compute features
        # features = torch.matmul(
        #     input.transpose(1, 2), mat.permute(0, 2, 3, 1)
        # ).transpose(1, 2)
        features = torch.matmul(input, mat.transpose(1, 2))
        # features = self.cv(features).squeeze(3)
        features = self.cv(features.unsqueeze(1))
